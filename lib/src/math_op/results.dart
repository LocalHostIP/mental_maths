...
class Results {
  ...
  List<OpRegister> subtraction =
      []; //All levels of substraction, each position of the list is a corresponding level
  ...
  void updateRegister(List<Operation> operations) {
    ...
    for (Operation op in operations) {
      if (op.operator == MathProblems.OPSum) {
        ...
        if (!updated[0].contains(op.level)) updated[0].add(op.level);
